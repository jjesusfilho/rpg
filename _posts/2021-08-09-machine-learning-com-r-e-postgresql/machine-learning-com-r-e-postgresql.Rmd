...
description: |
  Neste tutorial, irei mostrar como rodar kmeans, ou qualquer modelo, supervisionado ou não, estatístico ou de machine learning no PostgreSQL chamando o R. Com extensão PL/R isso é perfeitamente possível e fácil.
author:
...
Neste tutorial, irei mostrar como rodar um modelo estatístico ou de machine learning no PostgreSQL chamando o R. Com extensão PL/R isso é perfeitamente possível e fácil. Se você quiser saber mais sobre a extensão PL/R no PostgreSQL, acesse [este tutorial](https://direito.consudata.com.br/rpsql/plr/) ou o [guia do usuário](https://github.com/postgres-plr/plr/blob/master/userguide.md).

Um desafio a ser enfrentado é que modelos estatísticos e de machine learning geralmente retornam uma lista. No entanto, o PostgreSQL não reconhece listas do R. 

Para esse problema há duas soluções possíveis. Uma delas é usar o pacote broom, que converte grande parte dos resultados de modelos em dataframe. A outra, que acho mais elegante e flexível, é armazenar o modelo em formato json ou, melhor ainda, em jsonb no PostgreSQL. Nesse sentido, o pacote jsonlite vem a calhar, uma vez que, com ele, podemos converter listas para json e enviar para o PostgreSQL. Eventualmente, bastará  trazer o json do PostgreSQL para o R e  transformá-lo em lista novamente.

Para este tutorial, iremos mostrar como rodar kmeans. Naturalmente, você pode usar outro modelo, pois a lógica é a mesma. Usarei kmeans porque um amigo e ex-aluno trouxe o desafio dias atrás.
...
Ao receber a resposta da função, o PostgreSQL irá automaticamente convertê-la para jsonb. Nós iremos chamá-la do R mesmo a fim de que possamos verificar se o resultado pode ser usado como objeto kmeans. Naturalmente, você pode criar uma tabela no PostgreSQL e populá-la com o resultado.
